...
`/agent-memory-init` also deletes leftover `.cursor/agent-memory/AGENTS.md` (Conductor's old injected protocol) and leaves the rest of that directory alone.
...
- **Same store as Agent Conductor**: Uses `.cursor/agent-memory/<role>/`, so existing role stores keep working. Do not start a parallel `.cursor/memory/` tree.
