...
![How Agent Conductor routes context, compared with a broadcast rule](plugins/agent-conductor/docs/addressed-context.svg)
...
node scripts/validate-plugins.mjs        # manifests against the schemas
node scripts/validate-docs.mjs           # docs against what the code does
cd plugins/agent-conductor && python3 -m pytest
```

`validate-docs.mjs` exists because the marketing has drifted from the code twice. It reads the paths and config filenames the hooks actually resolve and fails when a document names something else.
